...
export default function Sidebar({user,view,setView,navItems,lang}){
  const roleColor={farmer:"var(--green)",retailer:"var(--gold)",delivery:"var(--blue)"};
    ...
    <aside style={{width:220,flexShrink:0,background:"#fff",borderRight:"1px solid var(--border)",display:"flex",flexDirection:"column",position:"sticky",top:100,height:"calc(100vh - 100px)",overflowY:"auto"}}>
      ...
      <div style={{padding:"20px 16px 16px",borderBottom:"1px solid var(--bg2)"}}>
        ...
        <div style={{fontSize:11,color:"var(--text3)",marginTop:4,display:"flex",flexDirection:"column",gap:2}}>
          ...
          <span>📍 {tLocation(user.village, user.district, lang)}</span>
        </div>
        <div style={{marginTop:8,display:"flex",alignItems:"center",gap:5,background:"var(--green-xp)",padding:"4px 8px",borderRadius:6,fontSize:11,color:"var(--green)",fontWeight:600}}>
          <div style={{width:6,height:6,borderRadius:"50%",background:"#22c55e",animation:"pulse 1.4s infinite"}}/>
          {pick(lang,"Online · Data saved locally","ಆನ್‌ಲೈನ್ · ಡೇಟಾ ಸ್ಥಳೀಯವಾಗಿ ಉಳಿಸಲಾಗಿದೆ")}
        </div>
      </div>
